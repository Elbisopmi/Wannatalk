...
public class AIMLProcessor {
    ...
    private static void categoryProcessor(Node n, ArrayList<Category> categories, String topic, String aimlFile, String language, LongLexTo thai_tokenize) {
        ...
        if( language.equals("th")) {
        	//TODO:
        	pattern = tokenize_thai(pattern);
        	that = tokenize_thai(that);
        	topic = tokenize_thai(topic);
        }
...
;
    ...
    public static String tokenize_thai(String str) {
    	String request="";
        tokenizer.wordInstance(str);
    	int beginIndex = tokenizer.first();
        while(tokenizer.hasNext()) {
        	int endIndex =tokenizer.next();
        	request += str.substring(beginIndex, endIndex)+" ";
        	beginIndex = endIndex;
        }
        return request;
    }
     ...
     */
    public static ArrayList<Category> AIMLToCategories (String directory, String aimlFile, LongLexTo thai_tokenize) {
        try {
            ...
            for (int i = 0; i < nodelist.getLength(); i++)   {
                ...
                if (n.getNodeName().equals("category")) {
                    categoryProcessor(n, categories, "*", aimlFile, language, thai_tokenize);
                }
                    ...
                    for (int j = 0; j < children.getLength(); j++) {
                        ...
                        if (m.getNodeName().equals("category")) {
                            categoryProcessor(m, categories, topic, aimlFile, language, thai_tokenize);
                        }
                    }
                }
            }
